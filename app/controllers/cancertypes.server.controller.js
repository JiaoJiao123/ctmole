...
exports.list = function (req, res) {
    Cancertype.find().sort('-created').populate('user', 'displayName').exec(function (err, cancertypes) {
        if (err) {
            return res.status(400).send({
                message: errorHandler.getErrorMessage(err)
            });
 ...
 */
exports.cancertypeByID = function (req, res, next, cancertypeSymbol) {
    Cancertype.findOne({'symbol': cancertypeSymbol}).populate('user', 'displayName').exec(function (err, cancertype) {
        if (err)
            return next(err);
        if (!cancertype)
            return next(new Error('Failed to load Cancertype ' + cancertypeSymbol));
...
exports.cancertypeByNctIds = function (req, res, next, ids) {
    ...
    Cancertype.find({nctIds: {$in: ids}}, {'_id': 0, 'symbol': 1}).populate('user', 'displayName').exec(function (err, cancertypes) {
        if (err)
            return next(err);
        if (!cancertypes)
            return next(new Error('Failed to load Cancertype ' + ids));
...
//				});
                Cancertype.find().stream()
                        .on('data', function (cancerType) {
                            overlappingNctIds = _.intersection(validStatusTrials, cancerType.nctIds);
                            tempIndex = cancertypesCount.map(function (e) {
                                return e.cancer;
                            }).indexOf(cancerType.cancer);
                            if (tempIndex === -1) {
                                cancertypesCount.push({cancer: cancerType.cancer, nctIds: overlappingNctIds});
                            } else {
                                cancertypesCount[tempIndex].nctIds = _.union(cancertypesCount[tempIndex].nctIds, overlappingNctIds);
                            }
                        })
